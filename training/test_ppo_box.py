...
register(id = "HumanBigTray-v0", entry_point = "load_xml:HumanBigTray")

env = gym.make('HumanBigTray-v0')
...
torch.save(ppo.pi.state_dict(), "pi")
torch.save(ppo.v.state_dict(), "v")
#ppo.v.load_state_dict(torch.load("v"))
#ppo.pi.load_state_dict(torch.load("pi"))
...
env = gym.make('HumanBigTray-v0', render_mode="human", terminate_when_unhealthy=True)
rewards = []
with torch.no_grad():
   while True:
      state, _ = env.reset()
